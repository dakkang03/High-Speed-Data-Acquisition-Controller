...
logic [31:0] adc_start_time_arr [NUM_CHANNELS-1:0];
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        throughput_counter <= 0; throughput_window_counter <= 0;
        samples_in_window <= 0; timestamp_1sec_ago <= 0; throughput_sps_reg <= 0;
    end else begin
        if (sample_valid) throughput_counter <= throughput_counter + 1;
...
always_comb begin
    if (throughput_window_counter > 0) throughput_sps = throughput_sps_reg;
    else if (sample_timestamp > 10000) throughput_sps = (throughput_counter * CLK_FREQ_HZ) / sample_timestamp;
    else throughput_sps = 0;
end

always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        for (int i = 0; i < NUM_CHANNELS; i++) begin
            sample_start_time[i] <= 0; latency_sum[i] <= 0; latency_count[i] <= 0; latency_max[i] <= 0;
        end
        current_latency <= 0;
    end else begin
        if (sample_valid && sample_channel < NUM_CHANNELS)
        ...
        if (fifo_rd_en) begin
            ...
            if (sample_start_time[read_channel] != 0) begin
                ...
                if (current_latency > latency_max[read_channel]) latency_max[read_channel] <= current_latency;
...
always_comb begin
    total_latency_sum = 0; total_latency_count = 0; global_max_latency = 0;
    for (int i = 0; i < NUM_CHANNELS; i++) begin
        total_latency_sum   = total_latency_sum + latency_sum[i];
        total_latency_count = total_latency_count + latency_count[i];
        if (latency_max[i] > global_max_latency) global_max_latency = latency_max[i];
    end
    avg_latency_ns = (total_latency_count > 0) ? (total_latency_sum / total_latency_count) : 0;
    max_latency_ns = global_max_latency;
end

always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        fifo_util_sum <= 0; fifo_util_samples <= 0; fifo_util_current <= 0;
    end else begin
        fifo_util_current <= (fifo_depth > 0) ? (fifo_count * 100) / fifo_depth : 8'd0;
...
always_comb begin
    ...
    end else fifo_utilization_pct = fifo_util_current;
end

always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin trigger_count_total <= 0; sample_count_total <= 0; end
    else begin
        if (trigger_detected) trigger_count_total <= trigger_count_total + 1;
        if (sample_valid) sample_count_total <= sample_count_total + 1;
    end
end
...
always_comb begin
    ...
    end else trigger_rate_ppm = 0;
end

always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        for (int i = 0; i < NUM_CHANNELS; i++) adc_start_time_arr[i] <= 0;
        adc_timeout_count <= 0;
    end else begin
        if (adc_conversion_start && adc_channel < NUM_CHANNELS)
            adc_start_time_arr[adc_channel] <= sample_timestamp;
        if (adc_conversion_done && adc_channel < NUM_CHANNELS) begin
            if (adc_start_time_arr[adc_channel] != 0)
                adc_start_time_arr[adc_channel] <= 0;
        end
        for (int i = 0; i < NUM_CHANNELS; i++) begin
            if (adc_start_time_arr[i] != 0 &&
                (sample_timestamp - adc_start_time_arr[i]) > (CLK_FREQ_HZ / 1000)) begin
                adc_timeout_count <= adc_timeout_count + 1;
                adc_start_time_arr[i] <= 0;
            end
        end
    end
end
...
always_comb begin
    ...
    warning_flags = 8'b0; flag_count = 0;
    if (throughput_sps < 2400 && throughput_sps > 0) begin warning_flags[WARN_LOW_THROUGHPUT]=1'b1; flag_count++; end
    if (avg_latency_ns > 1_000_000) begin warning_flags[WARN_HIGH_LATENCY]=1'b1; flag_count++; end
    if (fifo_utilization_pct > 80) begin warning_flags[WARN_HIGH_FIFO_USAGE]=1'b1; flag_count++; end
    if (fifo_full && fifo_wr_en) begin warning_flags[WARN_FIFO_OVERFLOW]=1'b1; flag_count++; end
    if (trigger_rate_ppm > 100_000) begin warning_flags[WARN_HIGH_TRIGGER_RATE]=1'b1; flag_count++; end
    else if (trigger_rate_ppm < 100 && sample_count_total > 1000) begin warning_flags[WARN_LOW_TRIGGER_RATE]=1'b1; flag_count++; end
    if (adc_timeout_count > 0) begin warning_flags[WARN_ADC_TIMEOUT]=1'b1; flag_count++; end
    if (flag_count >= 3) warning_flags[WARN_SYSTEM_OVERLOAD] = 1'b1;
end

assign debug_counters = {throughput_counter[15:0], trigger_count_total[15:0]};
